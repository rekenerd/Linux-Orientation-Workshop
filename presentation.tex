...
% Sets the continuation text for frametitle, in case of framebreaks
...
\author[\copyright GNU Free Document Licnese]{
Abdulkarim Memon (VIT, Final Year)\\
Akshay Mankar (VIIT, Alumni)\\
Imran Ahmed (JSCoE, Alumni)\\
Miheer Vaidya (VIT, Final Year)\\
Prathamesh Sonpatki (VIT, Alumni)}
...
\frame[allowframebreaks]{\frametitle{Outline}\tableofcontents}
...
\frame{\frametitle{Compile and Run}
	\begin{itemize}
		...
		\item Syntax: \alert{gcc [options] input\_file.c}
		\item Options
		\begin{itemize}
			\item \alert{-o} : Output File
			\item \alert{-Wall} : Show all warnings
			\item \alert{-c} : Compile Only
		\end{itemize}
		\item Example \\
...
\frame{\frametitle{Debugging Programs}
	\begin{itemize}
		\item Debugger used: GNU Debugger (gdb)
		\item Required For Debugging:
		\begin{itemize}
			\item \alert {CPU Instructions} : Required to execute
			\item \alert {Symbol Table} : Required to map higher level code with binary instructions
		\end{itemize}
		\item \alert {\$ gcc -o sum sum.c} will generate only Binary Executable
		\item Use option \alert {-g } to generate binary with symbol table
		\item So the command is\\
		\alert {\$ gcc -g -o sum sum.c}
		\item Start the Debugger by\\
		\alert {\$ gdb sum}
	\end{itemize}
}
\subsection{Debugger Operations}
\frame{\frametitle{Debugger Operations}
	\begin{itemize}
		\item \alert {break} : Set Breakpoints
		\item \alert {run} : Start Execution
		\item \alert {step} : Step into
		\item \alert {next} : Step over
		\item \alert {print} : Evaluate an expression
		\item \alert {display} : Set Watch
		\item \alert {undisplay} : Unset Watch
		\item \alert {continue} : Continue to next breakpoint or till end of program
	\end{itemize}
}
\subsection{Debug Tricks}
\frame{\frametitle{Debug Tricks}
	\begin{itemize}
		\item \alert b = break
		\item \alert r = run
		\item \alert s = step
		\item \alert n = next
		\item \alert p = print
		\item Enter \alert{blank command} to repeat last command
		\item Get help:\\
		\alert {http://sourceware.org/gdb/current/onlinedocs/gdb/}
	\end{itemize}
}
\subsection{ddd}
\frame{\frametitle{DDD - Data Display Debugger}
	\begin{itemize}
		\item A Simple to use GUI for gdb.
		\item Also has gdb shell to support programmers
	\end{itemize}
}
\frame{\frametitle{DDD - Data Display Debugger}
	\begin{figure}[htp]
		\centering
		\includegraphics[scale=0.5]{images/ddd.png}
	\end{figure}
}
\section{Communication Guidelines}
\subsection{Mailing Lists}
\frame{\frametitle{Mailing Lists}
\begin{itemize}
	\item \textbf{PLUG}
	http://www.plug.org.in/mailman/listinfo/plug-mail\\
	Pune Linux Users Group
	\item \textbf{CoFSUG}
	groups.google.com/group/cofsug\\
	CoEP Free Software Users Group
\end{itemize}
}
\subsection{IRC}
\frame{\frametitle{IRC}
\begin{itemize}
	\item \textbf{\#ubuntu} - Ubuntu Community
	\item \textbf{\#python} - Python programmong language
	\item \textbf{\#\#c} - C programming language
	\item \textbf{\#viitlug} - VIIT Linux Users group
\end{itemize}
}
